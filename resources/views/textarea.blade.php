...
<textarea id="{{ $id }}" name="{{ $name }}" {{ $disabled }}
    {!! $attributes->merge(['class' => "px-3 py-2 w-full min-h-10 text-gray-600 border $border rounded-md focus:ring focus:ring-gray-300 focus:ring-opacity-50"]) !!}>{{ $value }}</textarea>
